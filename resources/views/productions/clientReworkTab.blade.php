            ...
            <div class="card card-custom custom-top-border">
                <div class="card-body py-0 px-7">
                    ...
                    <div class="table-responsive pt-5 pb-5 clietnts_table">
                        <table class="table table-separate table-head-custom no-footer dtr-column " id="client_rework_list">
                            <thead>
                                             ...
                                             <div class="form-group row">
                                                ...
                                                <div class="col-md-10">
                                                    {!! Form::Select(
                                                        ...
                                                        null,
                                                        [
                                                            'class' => 'form-control white-smoke  kt_select2_qa_required_sampling pop-non-edt-val ',
                                                            'autocomplete' => 'none',
                                                            'id' => 'qa_required_sampling',
                                                            'style' => 'cursor:pointer',
                                                        ],
                                                    ) !!}
                                                </div>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.clickable-row', function(e) {
                ...
                $row.closest('table').find('thead th input').each(function() {
                    ...
                    function handleClientRevokData(clientData,headers) {
                            ...
                            if (/_el_/.test(value)) {
                                ...
                                if ($('input[name="' + header + '[]"][type="checkbox"]').length > 0) {
                                   ...
                                   $('select[name="' + header + '[]"]').val(value).trigger('change');
                                    ...
                                    } else if (header === 'claim_status'){
                                        claimStatus = value;console.log(value,header,'header',claimStatus);
                                            $('select[name="claim_status"]').val(claimStatus).trigger('change');
                                        $('#title_status').text(claimStatus);
                                    }
                                    if (header == 'id') {
            ...
            $(document).on('click', '.sop_click', function(e) {
                ...
                $(document).on('click', '#project_revoke_save', function(e) {
                    ...
                    var fieldNames = $('#revokeFormConfiguration').serializeArray().map(function(input) {
                        return input.name;
                    });
                    var requiredFields = {};
                    ...
                    $('#revokeFormConfiguration').find(':input[required], select[required], textarea[required]',
                    ...
                    $.each(fieldValuesByFieldName, function(fieldName, fieldValues) {
                        $.each(fieldValues, function(index, value) {
                            $('<input>').attr({
                                ...
                                name: fieldName + '[]',
                                value: value
                            }).appendTo('form#revokeFormConfiguration');
                        });
                    });
                    var claimStatus =  $('#claim_status').val();
                        if(claimStatus == "CE_Hold") {
                            var ceHoldReason = $('#ce_hold_reason');console.log(ceHoldReason.val(),'ceHoldReason');
                            if(ceHoldReason.val() == '') {
