                                            ...
                                            <tr>
                                                @if ($duplicateProjectDetails->contains('key', 'value'))
                                                    @foreach ($duplicateProjectDetails[0]->getAttributes() as $columnName => $columnValue)
                                                        @php
                                                            $columnsToExclude = ['id','CE_emp_id','duplicate_status','ce_hold_reason','qa_hold_reason','qa_work_status','QA_required_sampling','QA_rework_comments','coder_rework_status','coder_rework_reason','coder_error_count','QA_followup_date','CE_status_code','CE_sub_status_code','CE_followup_date', 'created_at', 'updated_at', 'deleted_at'];
                                                        @endphp
                                                ...
                                                @foreach ($duplicateProjectDetails as $data)
                                                    <tr>
                                                        ...
                                                        @foreach ($data->getAttributes() as $columnName => $columnValue)
                                                            @php
                                                                $columnsToExclude = ['id','CE_emp_id','duplicate_status','ce_hold_reason','qa_hold_reason','qa_work_status','QA_required_sampling','QA_followup_date','CE_status_code','CE_sub_status_code','CE_followup_date', 'created_at', 'updated_at', 'deleted_at'];
                                                            @endphp
                                                            @if (!in_array($columnName, $columnsToExclude))
        ...
        $(document).ready(function() {
            ...
            var table = $("#client_duplicate_list").DataTable({
                ...
                buttons: [{
                    ...
                    "title": 'PROCODE',
                    "filename": 'procode_qa_duplicate_'+date,
                    "exportOptions": {
