    ...
    public function qualitySampling(Request $request) {
            if (Session::get('loginDetails') &&  Session::get('loginDetails')['userDetail'] && Session::get('loginDetails')['userDetail']['emp_id'] !=null) {
                try {
                    ...
                    $client = new Client();
                    $response = $client->request('POST', 'http://dev.aims.officeos.in/api/v1_users/get_coder_emp_list', [
                        'json' => $payload
                    ]);
                    ...
                    $qaResponse = $client->request('POST', 'http://dev.aims.officeos.in/api/v1_users/get_qa_emp_list', [
                        'json' => $payload
                    ]);
